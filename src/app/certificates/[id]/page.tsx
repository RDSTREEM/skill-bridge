...
export default async function CertificatePage({ params }: { params: { id: string } }) {
  ...
  // Load SVG template
  const svgTemplate = await fs.readFile(process.cwd() + '/public/certificate-template.svg', 'utf8');
  // Replace placeholders
  const svg = svgTemplate
    .replace('{{name}}', cert.username || cert.email)
    .replace('{{extra}}', cert.challengeTitle || cert.status || '')
    .replace('{{status}}', cert.status || 'Beginner')
    .replace('{{applied}}', cert.challengesApplied?.toString() || '0')
    .replace('{{accepted}}', cert.challengesAccepted?.toString() || '0')
    .replace('{{completed}}', cert.challengesCompleted?.toString() || '0');
