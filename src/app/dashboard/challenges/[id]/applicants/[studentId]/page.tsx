...
export default function ApplicantEssayPage({ params }: { params: { id: string; studentId: string } }) {
  ...
  useState(() => {
    (async () => {
      ...
      if (
        ...
        challenge?.submissionStart && challenge?.submissionDeadline &&
        now >= challenge.submissionStart && now <= challenge.submissionDeadline
      ) {
  ...
  const handleSubmitEssay = () => {
    router.push(`/dashboard/challenges/${challengeId}/essay?studentId=${studentId}`);
  };
  ...
  const handleSubmitWork = () => {
    router.push(`/dashboard/challenges/${challengeId}/submit?studentId=${studentId}`);
  };
  ...
  return (
    <div className="max-w-xl mx-auto mt-8">
      ...
      <div className="mb-4 p-4 border rounded bg-gray-50">
        <div className="font-semibold mb-2">{student ? `${student.username} (${student.email})` : `Student ID: ${studentId}`}</div>
        <div><span className="font-medium">Essay:</span> <span className="italic">{essay}</span></div>
        <div className="mt-2 text-xs">Status: <b>{status}</b></div>
        {mentor && (
          <div className="mt-2 text-xs text-gray-600">By <span className="font-semibold">{mentor.username}</span></div>
        )}
        {error && <div className="text-red-600 text-xs mt-2">{error}</div>}
      </div>
      <div className="flex gap-4 mb-6">
        <Button onClick={() => handleDecision('accepted')} disabled={status === 'accepted' || loading}>Accept</Button>
        <Button onClick={() => handleDecision('rejected')} disabled={status === 'rejected' || loading}>Reject</Button>
        {canSubmitEssay && (
          <Button onClick={handleSubmitEssay} disabled={loading}>Submit Essay</Button>
        )}
        {canSubmit && (
          <Button onClick={handleSubmitWork} disabled={loading}>Submit Work</Button>
        )}
        {status === 'accepted' && !canSubmit && !canSubmitEssay && !completionRequested && (
          <Button onClick={handleMarkComplete} disabled={loading}>Mark as Complete</Button>
        )}
        {status === 'accepted' && completionRequested && (
          <span className="text-green-600 font-semibold">Completion requested</span>
        )}
      </div>
      <Link href={`../`} className="text-blue-600">&larr; Back to Applicants</Link>
    </div>
  );
}
