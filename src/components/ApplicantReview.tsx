      ...
      <div className="flex-1">
        {selected ? (
          <div className="border rounded p-4 bg-gray-50">
            <div className="font-semibold text-lg mb-1">{users[selected]?.displayName || users[selected]?.email || selected}</div>
            <div className="text-xs text-gray-500 mb-2">{users[selected]?.email}</div>
            <div className="mb-2"><span className="font-medium">Essay:</span> <span className="italic">{applicantsEssay?.[selected]}</span></div>
            <div className="flex gap-2 items-center mt-2">
              <span className="text-xs">Status: <b>{applicantsStatus?.[selected] || 'pending'}</b></span>
              <Button onClick={() => onDecision(selected, 'accepted')} disabled={applicantsStatus?.[selected] === 'accepted'}>Accept</Button>
              <Button onClick={() => onDecision(selected, 'rejected')} disabled={applicantsStatus?.[selected] === 'rejected'}>Reject</Button>
            </div>
          </div>
        ) : (
          <div className="text-gray-500">Select an applicant to review their essay.</div>
        )}
      </div>
    </div>
  );
